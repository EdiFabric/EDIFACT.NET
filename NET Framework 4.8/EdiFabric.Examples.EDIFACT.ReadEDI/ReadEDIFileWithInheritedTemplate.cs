...
namespace EdiFabric.Examples.EDIFACT.ReadEDI
{
    class ReadEDIFileWithInheritedTemplate
    {
        ...
        public static void Run()
        {
            ...
            //  1.  Load to a stream 
            Stream ediStream = File.OpenRead(Directory.GetCurrentDirectory() + Config.TestFilesPath + @"\Edifact\PurchaseOrderCustom2.txt");

            //  2.  Read all the contents
            List<IEdiItem> ediItems;
            using (var ediReader = new EdifactReader(ediStream, (UNB unb, UNG ung, UNH unh) => typeof(TSORDERSPartnerC).GetTypeInfo()))
